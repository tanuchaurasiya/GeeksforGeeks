...
pair<int,int> help(Node* root, int &ans){
    ...
    if(root->left || root->right)
        ans =  max(ans, max(root->data-min(left.first, right.first), root->data - max(left.second, right.second)));
    // cout<<"ans="<<ans<<endl;    
    // cout<<"return "<<min(root->data, left.first)<<" "<< max(root->data, right.second)<<endl;
    return {min(root->data, min(left.first, right.first)), max(root->data, max(left.second,right.second))};
